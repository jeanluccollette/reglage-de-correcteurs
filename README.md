...
Le programme [regl_corr.py](code/regl_corr.py) rassemble des fonctions réalisant le calcul des coefficients pour différents correcteurs et la validation de leur fonctionnement. Le programme [test_regl_corr.py](code/test_regl_corr.py) fait appel à ces fonction pour une fonction de transfert $\mu(p)$ imposée. Ces programmes utilisent le package [control](https://python-control.readthedocs.io/en/0.10.1/) (on choisira de préférence la version 0.10.1).
